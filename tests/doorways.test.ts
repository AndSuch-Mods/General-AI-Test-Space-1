...
import { canInteract, canStand, doorEntry, getRoomObjects, nearestInteractable, roomLayout } from '../src/content/room';
import { placementError } from '../src/content/furnishing';

describe('wall-aligned household routes', () => {
  it.each(['castle', 'bedroom-2', 'living', 'landing'] as const)('returns through the opposite wall for every doorway in %s', async map => {
  ...
  it('repairs an obstructed legacy arrival without moving furniture or losing saved progress', async () => {
    ...
    expect({ x: p.x, y: p.y }).not.toEqual({ x: 132, y: 454 }); expect(a.world.roomLayouts).toEqual(world.roomLayouts);
    expect(p.discoveries).toEqual(['letter']);
  });
  it('reserves the left, right and lower living-room approaches during arrangement', () => {
    const world = createWorld(createPlayer('Keeper'));
    for (const offset of [{ x: -656, y: 42 }, { x: 0, y: 42 }, { x: -312, y: 42 }]) {
  ...
  it('doubles every window while keeping it above the floor and clear of doors', () => {
    for (const map of ['castle', 'bedroom-2', 'living', 'landing'] as const) {
