...
Individual ember releases last 130–430 ms, with 28–58 ms attacks. Deterministic variation changes their spacing, noise offset, cutoff and level. They sit inside the continuous bed instead of making a repeated sequence of isolated, sharp clicks. Ignition is a breath of air and flame; extinguishing has a soft hiss. The fire follows the local hearth flag in bedrooms and the living room, or the stove flag in the kitchen, and is absent from the entry hall. Music mute leaves these room sounds available. The existing public audio API is unchanged.
...
The current development build passed two full sound off/on cycles while offline in Chromium. With music muted, the kitchen stove produced a live signal, faded to silence when extinguished, and restarted when relit. Leaving that burning room did not carry its fire ambience into the title. Separate lifecycle checks confirmed that mute and suspension remove every active fire voice and scheduler.
