...
The isolated `.local/eye-hotfix` checkout and `codex/eye-correction`, checkpoint `7b1539b02bab32ce1d3061484de73f46f1533edc`, record the original correction. The combined household restoration now retains its matched open eyes in the registered character layers. Do not reset or overwrite the integrated household work to match that historical hotfix. Current validation is recorded in `VALIDATION.md`.
...
Checkpoint `7b1539b` subsequently passed all 21 browser cases on each engine in `35801560189`, including real connected second peers in the diagnostic logs. It was promoted to main for the isolated eye release. Release run `35802015049` passed both browser jobs and deployed successfully. These results do not establish the cause of the earlier intermittent reconnect failures.
