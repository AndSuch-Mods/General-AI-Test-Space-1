...
export function residentTextureKey(appearance: ResidentAppearance, look: CharacterLook = DEFAULT_LOOK): string {
  return `resident-raster-v8-${appearance}-${look.body}-${look.hairStyle}-${look.hairColor}-${look.skinTone}-${look.outfit}`;
}
...
/** Shared, read-only native artwork assembled from registered raster layers. */
...
/** Compatibility signature: packed raster masters need no image preload. */
